...
#'           type = "instantaneous",
#'           datapoints = "latest")
loadAPIdata <- function(ID = NULL, measure = NULL, period = NULL,
  ...
  if (ID[1] == "flood" &&
    ...
    ## Change baselink
    baselink <- "http://environment.data.gov.uk/flood-monitoring/id/stations.csv"
    baselinkJ <- "http://environment.data.gov.uk/flood-monitoring/id/stations.json"
  ...
  if (ID[1] == "tidal" &&
    ...
    ## Limit set to 20,000 (current API is ~5,000) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## Change baselink
    baselink <- "http://environment.data.gov.uk/flood-monitoring/id/stations.csv?type=TideGauge"
    baselinkJ <- "http://environment.data.gov.uk/flood-monitoring/id/stations.json?type=TideGauge"
  ...
  if ((is.null(ID) || ID[1] %in% c("all", "wiski", "nrfa")) &
    ...
    datalink <- gsub("json", "csv", datalink)
    dt <- data.table::fread(datalink)
    ## Unnesting observed property is slightly more difficult ~~~~~~~~~~~~~~~~~~
    ## Stored as dataframe list ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ## Remove URL and convert dataframes to string ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  if (!is.null(ID) & !is.null(measure) & !is.null(period)) {
    ...
    if (length(refs$ID) > 1) {
      ...
      cli::cli_h3(" Data references used in download")
      print(refs)
      ## Iterate through IDs
