...
#' @rdname dayStats
#' @export
dayStats.data.table <- function(x = NULL,
                                methods = c("mean", "median", "min", "max"),
                                percentiles = c(5, 25, 75, 95),
                                hydroDay = TRUE,
                                plot = TRUE,
  ...
  if (plot == TRUE) {
    ...
    polygon(c(a$dayYear, rev(a$dayYear)), c(a$Max, rev(a$Min)),
            col = "royalblue3")
    polygon(c(a$dayYear, rev(a$dayYear)), c(a$Perc5, rev(a$Perc95)),
            col = "royalblue1")
    polygon(c(a$dayYear, rev(a$dayYear)), c(a$Perc25, rev(a$Perc75)),
            col = "lightblue")
