...
#' @param gapWidth This allows you to ignore a set number of timesteps beneath a
#'  threshold between two or more events
...
#                     .(Location, Value),]
#
# yorkVik <- loadAPI(ID = 'L2406', measure = 'level',
#                    period = 900,
#                    type = 'instantaneous',
#                    datapoints = 'range',
...
#
#
# dt <- exceed(yorkVik$data$dateTime,
#              yorkVik$data$value,
#              # threshold = deets$Value,
#              threshold = 2.5,
#              gapWidth = 0)
#
# dt <- data.table(dt, hydroYearDay(dt$Start))
# counts <- dt[, .(count = .N, sum = sum(timeSteps)), by = hydroYear]
# # Remove error
# counts <- counts[hydroYear >= 1997,]
#
#
# counts %>% dcast(hydroYear ~ shortName,
#              value.var = "actualWait")
#
# piv <- tidyr::pivot_longer(counts, cols = c(count, sum))
#
# ggplot(piv, aes(hydroYear, value)) +
#   geom_col() +
